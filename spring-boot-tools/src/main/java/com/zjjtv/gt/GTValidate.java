package com.zjjtv.gt;


import com.zjjtv.utils.IpUtil;
...
public class GTValidate {
    ...
    public static boolean validateGT (HttpServletRequest request) {
        ...
        GeetestLib gtSdk = new GeetestLib(PropertiesUtils.APP.getProperty("geetest_id"), PropertiesUtils.APP.getProperty("geetest_key"), true);
        //RedisService redisService = SpringUtil.getBean("redisService");
        //从session中获取gt-server状态
        String status = "";//redisService.get("gt:" + Ip + ":gt_server_status");
        if (StringUtils.isEmpty(status)) {
            System.out.println("The Gt Server Status not exist in redis");
            return false;
        }
